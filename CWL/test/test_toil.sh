...
cd $TMP_DIR
cwltoil --logDebug --maxCores 10  --disableCaching --jobStore=`pwd`/jobstore --workDir=/local_tmp/ --basedir=`pwd`/base --outdir=`pwd`/out --batchSystem=gridEngine ${SRC_DIR}/../wgbs-workflow-pilot.yaml ${SRC_DIR}/../test/test-job-wfl-${2}.yml
